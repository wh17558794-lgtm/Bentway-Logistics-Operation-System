...
assert.match(source, /const DEFAULT_CRANE_TRUCK_FEE = 810;/);
assert.match(source, /shipment\.crane_truck_fee \?\? DEFAULT_CRANE_TRUCK_FEE/);
assert.match(source, /event\.target\.name === 'crane_required'[\s\S]*?craneFeeField\.value = String\(DEFAULT_CRANE_TRUCK_FEE\)/);
...
assert.doesNotMatch(operationsCss, /\.sidebar:hover/);
assert.match(source, /sidebar\.addEventListener\('click'[\s\S]*?classList\.add\('desktop-open'\)/);
assert.match(source, /sidebar\.addEventListener\('mouseleave'[\s\S]*?classList\.remove\('desktop-open'\)/);
...
assert.match(source, /pendingTableMode: 'x'/);
assert.match(source, /x: \['QTY', 'Suburb', 'Tracking Number', 'Customer Reference', 'Name', 'Address', 'Contact Phone', 'Inbound Time', 'Container Number', 'Status'\]/);
assert.match(source, /y: \[[^\]]*'Total Charge', 'Agent Customer'\]/);
...
assert.match(source, /pendingTableMode === 'x' \? 'y' : 'x'/);
...
assert.match(source, /pendingTableMode === 'y'[\s\S]*?status-action[\s\S]*?: `<td class="status-cell"><div class="status-cell-content">/);
assert.match(html, /Switch to Y table layout/);
...
assert.match(source, /p_status: 'pending'/);
...
assert.match(source, /\['completed', 'cancelled', 'exception'\]\.includes\(item\.current_status\)/);
...
assert.match(html, /Billing &amp; Service Details[\s\S]*?Name &amp; Delivery Details[\s\S]*?Shipment &amp; Storage Details[\s\S]*?Status &amp; Internal Notes/);
assert.match(source, /shipmentModal\.classList\.toggle\('billing-table-order', state\.activeView === 'pending' && state\.pendingTableMode === 'y'\)/);
assert.match(operationsCss, /#shipmentModal\.billing-table-order \.shipment-storage-section \{ order: 2; \}[\s\S]*?#shipmentModal\.billing-table-order \.name-delivery-section \{ order: 3; \}/);
assert.match(html, /Status &amp; Internal Notes[\s\S]*?name="current_status"[\s\S]*?name="exception_reason"[\s\S]*?name="exception_resolution_reason"[\s\S]*?name="notes"/);
...
assert.match(source, /p_notes: enteringException[\s\S]*?record\.exception_resolution_reason/);
...
assert.match(source, /function saveDispatchStatus\(event\)[\s\S]*?set_shipment_dispatch_status/);
...
assert.match(operationsCss, /data-table-mode="y"[\s\S]*?:nth-child\(5\)[\s\S]*?text-align: left/);
assert.match(operationsCss, /data-table-mode="y"[\s\S]*?:nth-child\(2\)[\s\S]*?text-align: left/);
assert.match(operationsCss, /data-table-mode="y"[\s\S]*?table-layout: fixed/);
